...
public class MonsterHealth : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        MonsterMove = GetComponent<MonsterAstar>();
    ...
    void PlayerDie()
    {
        ...
        GetComponent<MonsterAttack>().AllStop();
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if(collision.tag == "PlayerAttack")
        {
            ...
            if(!ani.GetBool("Walk")&& Time.time-MonsterMove.AttackDelay > 1f)
                ani.SetTrigger("Hit");
        }
        if (collision.tag == "PlayerFire2")
        {
            Destroy(collision.gameObject);
            TakeDamage(10, Player.transform.position, 0);
            GameObject fireefect = Instantiate(WormfireEffct, transform.position - new Vector3(0, 0.7f, 0), Quaternion.identity);
        ...
        if (collision.tag == "PlayerFire")
        {
            Destroy(collision.gameObject);
            TakeDamage(10, Player.transform.position, 0);
            GameObject fireefect = Instantiate(WizardfireEffct, transform.position -new Vector3(0,0.7f,0), Quaternion.identity);
            Destroy(fireefect, 0.5f);
            if (!ani.GetBool("Walk") && Time.time - MonsterMove.AttackDelay > 1f)
                ani.SetTrigger("Hit");
