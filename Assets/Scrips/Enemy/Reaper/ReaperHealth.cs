...
public class ReaperHealth : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        ReaperMove = GetComponent<ReaperAstar>();
    ...
    void PlayerDie()
    {
        ...
        GetComponent<ReaperAttack>().AllStop();
        ani.ResetTrigger("Hit");
        ani.SetBool("Die", true);
        //움직임 스크립트 없앰
        ReaperMove.enabled = false;
        Death = true;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            TakeDamage(10, Player.transform.position, 0.7f);
            //이동중이 아니고 && 공격중아니면 맞는모션 실행
            if (!ani.GetBool("Walk") && Time.time - ReaperMove.AttackDelay > 1f)
                ani.SetTrigger("Hit");
            if (!ReaperMove.Target)
            {
                ReaperMove.StartCoroutine("TargetOon");
